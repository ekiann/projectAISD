...
from ui_new_tests import Ui_NewTests
import router
import res_rc


class NewTests(QMainWindow):
    def __init__(self, info, parent=None):
        ...
        count = cursor.execute('SELECT COUNT(company_id) FROM tests WHERE company_id=?', (self.user_info[1], )).fetchone()[0]
        if count != 0:
            self.tests_info = cursor.execute('SELECT * FROM tests WHERE company_id=?', (self.user_info[1], )).fetchall()
        connection.close()
        grid = QVBoxLayout()
        ...
        for i in range(count):
            ...
            QPushButton:pressed{color: #FFFFFF; background-color: #FF8936; border-radius: 20;}''')
            ...
            label_title = QLabel()
            label_title.setText(self.tests_info[i][1])
            label_title.setStyleSheet('QLabel{color: #FFFFFF; font-size: 18pt; font-weight: bold;}')
            label_res = QLabel()
            label_res.setText(f'<b>{self.tests_info[i][3]}</b> вопросов')
            label_res.setStyleSheet('QLabel{color: #FFFFFF; font-size: 13pt;}')
            label_status = QLabel()
            label_status.setText(f'до <b>{self.tests_info[i][2]}</b>')
            ...
            button.clicked.connect(lambda checked, i=i: self.show_test_window(i))
        vertical_spacer = QSpacerItem(20, 40, QSizePolicy.Minimum, QSizePolicy.Expanding)
        grid.addItem(vertical_spacer)
        w = QWidget()
        w.setLayout(grid)
    ...
    def show_test_window(self, i):
        self.window = self.router.go_to_test(self.user_info, self.tests_info[i])
        self.window.show()
    ...
    def tests(self, event, t):
        ...
        if not complete.isHidden():
            ...
            self.timer.start(t)
            self.ui.arrow_button.setIcon(QIcon(':/new_test_img/arrow.png'))
            ...
            self.anim_new_text.start()
            self.anim_complete_text.start()
            self.ui.arrow_button.setIcon(QIcon(':/new_test_img/down_arrow.png'))
    ...
    def hide(self):
        ...
        self.ui.menu_button.setStyleSheet(
            ...
            background-image: url(:/new_test_img/menu.png); border: none;}''')
